...
use crate::{
    ...
    equipment_swapping::{ensure_mannequin_gear, MannequinGearMode},
...
#[cfg(windows)]
const STARTUP_REG_VALUE: &str = "SF Bot";
...
pub async fn singleAccountExecution(accountName: &str, pw: &str, charactername: &str, ziel: &str) -> Result<Vec<CharacterDisplay>, String>
{
    ...
    {
        let mut session = &mut sessionState.session; 
            ...
            Ok(gamestate) =>
                {
                    ...
                    add_session_state(SessionState {
                        ...
                        character_name: String::from(charactername.to_lowercase()), 
                        character_id: id,
                        server: accountName.to_string(),                            
...
pub async fn run_func(session: &mut SimpleSession, ziel: &str, isSingle: bool, server: &str) -> Result<Vec<CharacterDisplay>, Box<dyn std::error::Error>>
{
    let result = match ziel
    {
        ...
        "cmd_manage_inventory" => cmd_manage_inventory(session).await?, 
        ...
        _ => return Err(Box::new(std::io::Error::new(std::io::ErrorKind::InvalidInput, "Invalid command"))), 
...
pub async fn cmd_play_expeditions_exp(session: &mut SimpleSession) -> Result<String, Box<dyn std::error::Error>>
{
    ...
    {
        ensure_mannequin_gear(session, MannequinGearMode::Bonus).await?;
        play_expeditions_exp(session, &*name, skip_using_hourglasses, beers_to_drink as u8, priorities_list).await?;
        let gs_after = session.send_command(Command::Update).await?;
        if !matches!(gs_after.tavern.current_action, CurrentAction::Expedition) && gs_after.tavern.expeditions.active().is_none()
        {
            ensure_mannequin_gear(session, MannequinGearMode::Main).await?;
        }
    }
    Ok("".to_string())
}
...
fn set_start_on_boot(enable: bool) -> Result<(), String>
{
    ...
    let (key, _) = hkcu.create_subkey(STARTUP_REG_PATH).map_err(|e| e.to_string())?;
    ...
    if enable
    {
        ...
        key.set_value(STARTUP_REG_VALUE, &quoted_path).map_err(|e| e.to_string())?;
    }
    else
    {
        match key.delete_value(STARTUP_REG_VALUE)
        {
            Ok(_) => {}
...
{
    #[cfg(windows)]
    if let Some(enable) = settings.get("globalStartOnBoot").and_then(|v| v.as_bool())
    {
        set_start_on_boot(enable).map_err(|e| format!("Failed to update Windows startup setting: {}", e))?;
    }
...
pub async fn login_single_account(name: &str, pw: &str, single: bool, server: &str) -> Result<Vec<CharacterDisplay>, String>
{
    ...
    let session_state = SessionState {
        account_name: String::from(name).to_lowercase(),                 
        character_id: gamestate.character.player_id.clone(),             
        character_name: gamestate.character.name.clone().to_lowercase(), 
        server: String::from(server).to_lowercase(),                     
        session,
    };
