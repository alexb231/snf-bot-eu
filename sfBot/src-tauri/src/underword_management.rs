...
pub async fn perform_underworld_atk_suggested_enemy(session: &mut SimpleSession) -> Result<String, Box<dyn std::error::Error>>
{
    ...
    let updated = session.send_command(Command::UpdateLureSuggestion).await?.clone();
    if let Some(uw) = updated.underworld
    {
        if let Some(suggestion) = uw.lure_suggestion
        {
            let viewed = session.send_command(Command::ViewLureSuggestion { suggestion }).await?.clone();
            if let Some(target_name) = viewed.hall_of_fames.players.first().map(|player| player.name.clone())
            {
                let viewed_target = session.send_command(Command::ViewPlayer { ident: target_name.clone() }).await?.clone();
                if let Some(looked_up_player) = viewed_target.lookup.lookup_name(&target_name)
                {
                    ...
                    write_character_log(
                        ...
                        &format!(
                            "UNDERWORLD: Attack sent to suggested lure {} ({})",
                            looked_up_player.name, looked_up_player.player_id
                        ),
                    );
                }
            }
