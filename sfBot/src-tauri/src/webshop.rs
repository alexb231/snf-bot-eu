...
use crate::bot_runner::write_character_log;
...
pub struct CouponRedeemProgress {
    pub processed: usize,
    pub total: usize,
...
fn get_webshop_character_id_candidates(gs: &GameState) -> Vec<(&'static str, String)> {
    ...
    {
        ids.push(("webshop_id", id.to_string()));
    }
    if let Some(id) = gs
        .character
        .sf_home_id
        ...
        .filter(|id| !id.is_empty())
    {
        if !ids.iter().any(|(_, existing)| existing == id) {
            ids.push(("sf_home_id", id.to_string()));
        }
    }
    ids
}
...
fn is_set_server_success(response_body: &str) -> bool {
        ...
        return match value {
            ...
            Value::Object(map) => {
                map.get("success")
                    ...
                    .unwrap_or(false)
                    || map.get("auth").and_then(Value::as_bool).unwrap_or(false)
            }
            _ => false,
        };
    }
    ...
    Ok(catalog
        ...
        .and_then(|catalog| {
            catalog
                .articles
                .into_iter()
                .find(|article| article.sku.eq_ignore_ascii_case("FREE"))
        }))
}
...
pub async fn claim_free_mushroom(
    ...
    let char_id_candidates = get_webshop_character_id_candidates(&gs);
    if char_id_candidates.is_empty() {
        return Ok(String::new());
    }
    ...
    let mut active_char_id_raw: Option<String> = None;
    let mut active_char_id_source: Option<&'static str> = None;
    let mut last_error: Option<String> = None;
    let mut cookies: Vec<String> = Vec::new();
    let mut attempted_char_ids: Vec<String> = Vec::new();
    for (source, char_id) in char_id_candidates {
        attempted_char_ids.push(format!("{source}={char_id}"));
        ...
        .await
        {
            Ok((login_resp, set_cookies)) => {
                if is_set_server_success(&login_resp) {
                    active_char_id_encoded = Some(char_id_encoded);
                    active_char_id_raw = Some(char_id);
                    active_char_id_source = Some(source);
                    cookies = set_cookies;
                    break;
                }
    ...
    let Some(char_id_encoded) = active_char_id_encoded else {
        write_character_log(
            ...
            &format!(
                "WEBSHOP: setServer failed for character ids [{}]",
                attempted_char_ids.join(", ")
            ),
        );
    ...
    if let Some(char_id_raw) = active_char_id_raw {
        let source = active_char_id_source.unwrap_or("unknown");
        write_character_log(
            &gs.character.name,
            gs.character.player_id,
            &format!("WEBSHOP: setServer login using {source}={char_id_raw}"),
        );
    }
    ...
    if let Some(free_item) = fetch_free_article(cookie_header.as_deref()).await? {
        let claimable = free_item.valid_again_in_seconds.unwrap_or(0) <= 0;
        if claimable {
        ...
        let payment_string = match build_payment_string(&gs, server_id) {
            ...
            None => {
                result.message = format!(
                    "Missing player save id (player_id={}, save_id={})",
                    gs.character.player_id, gs.character.player_save_id
                );
                return result;
            }
        };
...
pub async fn redeem_coupon_for_all(
    ...
    redeem_coupon_for_all_with_progress(code, |_| {})
        .await
}

pub async fn redeem_coupon_for_all_with_progress<F>(
    code: &str,
    mut on_progress: F,
) -> WebshopResult<CouponRedeemSummary>
where
    F: FnMut(CouponRedeemProgress),
{
    let code = code.trim();
    if code.is_empty() {
    ...
    on_progress(CouponRedeemProgress {
        processed: 0,
        total,
        current: None,
    });
    ...
    for (idx, mut session_state) in session_states.into_iter().enumerate() {
        on_progress(CouponRedeemProgress {
            ...
            total,
            current: Some(session_state.session.username().to_string()),
        });
        ...
        on_progress(CouponRedeemProgress {
            processed: results.len(),
            total,
            current: results.last().map(|r| r.name.clone()),
        });
