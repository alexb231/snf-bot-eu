...
use sf_api::{
    ...
    gamestate::{
        character::Class,
        items::{BagPosition, EquipmentSlot, Gem, GemSlot, GemType, Item, ItemType, PlayerItemPlace, RuneType},
        GameState,
    },
    misc::EnumMapGet,
    SimpleSession,
...
fn total_attribute(gs: &GameState, attr: AttributeType) -> u32 { gs.character.attribute_basis[attr] + gs.character.attribute_additions[attr] + gs.character.attribute_times_bought[attr] }

fn item_attribute(item: &Item, attr: AttributeType) -> u32 { *item.attributes.get(attr) }
...
fn average_weapon_damage(item: &Item) -> Option<f64>
{
    match item.typ
    {
        ItemType::Weapon { min_dmg, max_dmg } => Some((f64::from(min_dmg) + f64::from(max_dmg)) / 2.0),
        _ => None,
    }
}
...
fn can_equip_item(gs: &GameState, item: &Item) -> bool { item.can_be_equipped_by(gs.character.class) }
...
fn get_item_value(gs: &GameState, current_item: Option<&Item>, item_to_check: Option<&Item>) -> f64
{
    ...
    let socket_boost = if current_item.is_some() && !item_has_socket(current_item.unwrap()) && item_has_socket(item_to_check) { 1.0 } else { 0.0 };

    let main_attr = gs.character.class.main_attribute();
    let current_item_value = current_item.map_or(0, |item| item_attribute(item, main_attr) + item_attribute(item, AttributeType::Constitution));
    let new_item_value = item_attribute(item_to_check, main_attr) + item_attribute(item_to_check, AttributeType::Constitution);
    ...
    {
        let ratio = current_item.and_then(average_weapon_damage).filter(|avg| *avg > 0.0).map(|avg| new_avg / avg).unwrap_or(0.2);
...
pub async fn check_and_swap_equipment(session: &mut SimpleSession) -> Result<String, Box<dyn std::error::Error>>
{
    ...
    let min_boost_percent: f64 = fetch_character_setting(&gs, "itemsEquipSwapMinBoostPercent").unwrap_or(0).max(0) as f64;
    ...
    {
        write_character_log(&gs.character.name, gs.character.player_id, &format!("EQUIP_SWAP: swapped={} slots=[{}]", changes.len(), swapped_slots.join(", ")));
        Ok(format!("Swapped equipment:\n{}", changes.join("\n")))
    }
}

fn get_item_ref_from(gs: &GameState, pos: BagPosition) -> Option<&Item> { gs.character.inventory.backpack.get(pos.backpack_pos()).and_then(|o| o.as_ref()) }
