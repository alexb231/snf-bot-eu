...
pub async fn train_fortress_units(session: &mut SimpleSession) -> Result<String, Box<dyn std::error::Error>>
{
    ...
    let units = fortress.units.clone(); 
    let current_resources = fortress.resources.clone(); 
...
pub async fn attack_fortress(session: &mut SimpleSession) -> Result<String, Box<dyn std::error::Error>>
{
    ...
    let soldier_advice = match player.fortress.as_ref().map(|ft| ft.soldier_advice)
    {
        Some(advice) => advice,
        None =>
        {
    ...
    let requested_soldiers = if soldier_advice == 0
    {
        1
    }
    else
    {
        (soldier_advice as f64 * multiplier).ceil() as u16
    };
    let final_attack_count = requested_soldiers.min(available_soldiers).min(max_soldiers);

    if final_attack_count == 0
    {
        return Ok("".to_string());
    }
    ...
    {
        match session.send_command(Command::FortressAttack { soldiers: final_attack_count as u32 }).await
        {
            Ok(_) => {}
            Err(SFError::ServerError(msg)) if msg == "need more units" =>
            {
                return Ok("".to_string());
            }
