...
use sf_api::{
    ...
    gamestate::{
        ...
        guild::Guild,
        items::{BagPosition, EquipmentSlot, GemSlot, GemType, Inventory, InventoryType, Item, ItemCommandIdent, ItemPlace, ItemPlace::MainInventory, ItemPosition, ItemType, PetItem, PlayerItemPlace, PlayerItemPosition, Potion, PotionSize, PotionType},
        rewards::Event,
...
pub async fn manage_inventory(session: &mut SimpleSession) -> Result<String, Box<dyn Error>>
{
    ...
    let exclude_epics_from_witch_selling: bool = fetch_character_setting(&gs, "itemsImmediatelyThrowIntoCauldronExceptEpics").unwrap_or(false); 
    ...
    let only_sell_after_toilet_use = should_delay_item_selling_until_toilet_used(&gs);

    let mut free_slots = gs.character.inventory.count_free_slots();
    drink_potions(session).await?;
    if !only_sell_after_toilet_use
    {
    ...
    if sell_items_to_witch && !only_sell_after_toilet_use
    {
    ...
    if only_sell_after_toilet_use
    {
        return Ok(String::from("Inventory full: waiting for toilet usage before selling."));
    }
...
fn should_delay_item_selling_until_toilet_used(gs: &GameState) -> bool
{
    let only_sell_after_toilet_use: bool = fetch_character_setting(&gs, "itemsSellAfterToiletUse").unwrap_or(false);
    if !only_sell_after_toilet_use
    {
        return false;
    }
    ...
    if !(throw_epics || throw_normal_items || throw_gems_only)
    {
        return false;
    }
    ...
    has_toilet_candidate_item(gs, throw_epics, throw_normal_items, throw_gems_only)
}

fn has_toilet_candidate_item(gs: &GameState, throw_epics: bool, throw_normal_items: bool, throw_gems_only: bool) -> bool
{
    let exclude_epic_weapons: bool = fetch_character_setting(&gs, "toiletExcludeEpicWeapons").unwrap_or(false);

    for (_bag_pos, maybe_item) in gs.character.inventory.iter()
    {
        ...
        if throw_gems_only && matches!(&item.typ, ItemType::Gem(_))
        {
            return true;
        }
        ...
        if throw_epics
        {
            ...
            if item.is_epic() && !is_weapon_with_filled_gem && !(exclude_epic_weapons && is_weapon)
            {
                return true;
            }
        }

        if throw_normal_items && !matches!(&item.typ, ItemType::Potion(_))
        {
            return true;
        }
    }
...
pub async fn buy_potions_and_hourglasses(session: &mut SimpleSession) -> Result<String, Box<dyn Error>>
{
    ...
    let new_pos = bag_to_playerplace(free_slot);
...
pub async fn check_whether_hourglas_is_in_inv(session: &mut SimpleSession) -> Result<(), Box<dyn Error>>
{
    ...
    for (bag_pos, item) in sorted_items_with_bagpos(inv)
    {
        if item.typ == ItemType::QuickSandGlass
        {
            let from = bag_to_itemplace(bag_pos);
            session.send_command(Command::UsePotion { from, item_ident: item.command_ident() }).await?;
        }
    }
    Ok(())
}
...
pub async fn drink_potions(session: &mut SimpleSession) -> Result<(), Box<dyn Error>>
{
    ...
    if amount_of_active_potions < 3 || all_pots_are_none
    {
        for (bag_pos, ident, item_type) in potion_to_drink
        {
            ...
            if let Err(err) = session.send_command(Command::UsePotion { from, item_ident: ident }).await
            {
            ...
            {
                write_character_log(&character_name, character_id, &format!("POTION: drank typ={:?} size={:?} pos={}", p.typ, p.size, from.position));
            }
        }
    }
    else
            ...
            if let ItemType::Potion(potion) = item_type
            {
                for pot in character_pots.iter().flatten()
                {
                    if pot.typ == potion.typ && pot.size == potion.size
                    {
                        ...
                        session.send_command(Command::UsePotion { from, item_ident: *ident }).await?;

                        write_character_log(&character_name, character_id, &format!("POTION: drank typ={:?} size={:?} pos={}", potion.typ, potion.size, from.position));
                    }
                }
            }
        }
...
pub async fn sell_potions(session: &mut SimpleSession) -> Result<(), Box<dyn Error>>
{
    ...
    {
        let item_pos = bag_to_playerplace(bag_pos);

        session.send_command(Command::SellShop { item_pos, item_ident: ident }).await?;

        if let ItemType::Potion(potion) = item_type
        {
            write_character_log(&character_name, character_id, &format!("SELL: potion typ={:?} size={:?} pos={}", potion.typ, potion.size, item_pos.position));
        }
    }
...
pub async fn sell_gems(session: &mut SimpleSession) -> Result<(), Box<dyn Error>>
{
    ...
    for (bag_pos, ident, item_type) in gems_to_sell
    {
        ...
        {
            write_character_log(&character_name, character_id, &format!("SELL: gem typ={:?} value={} pos={}", gem.typ, gem.value, item_pos.position));
        }
    }
        ...
        if witch_event_active
        {
            if exclude_epics_from_witch_selling
            {
                ...
                for bag_pos in items_to_drop
                {
                    ...
                    let drop_command = Command::WitchDropCauldron { item_pos };

                    session.send_command(drop_command).await?;

                    if let Some((item_typ, item_price)) = item_info
                    {
                        write_character_log(&character_name, character_id, &format!("WITCH: sold pos={} item={:?} price={}", item_pos.position, item_typ, item_price));
                    }
                }
...
pub async fn sell_two_cheapest_items(session: &mut SimpleSession, exclude_epics_from_witch_selling: bool) -> Result<String, Box<dyn Error>>
{
    ...
    for (bag_pos, ident, item_type, item_price) in items_to_sell
    {
        ...
        write_character_log(&gs.character.name, gs.character.player_id, &format!("SELL: item typ={:?} price={} pos={}", item_type, item_price, item_pos.position));
    }
...
pub fn find_required_item_for_witch<'a>(inventory: &'a Inventory, required_slot: EquipmentSlot, exclude_epics_from_witch_selling: bool) -> Option<(BagPosition, &'a Item)>
{
    ...
    let item_matches = |item: &ItemType| match (&required_type, item)
    {
        ...
        (ItemType::Weapon { .. }, ItemType::Shield { .. }) => true, 
...
pub fn bag_to_itemplace(bag: BagPosition) -> ItemPosition
{
    let (inv_type, pos) = bag.inventory_pos();
    ItemPosition {
        place: inv_type.item_position(),
        position: pos,
    }
}
...
pub fn bag_to_playerplace(bag: BagPosition) -> PlayerItemPosition
{
    let (inv_type, pos) = bag.inventory_pos();
    PlayerItemPosition {
        place: inv_type.player_item_position(),
        position: pos,
    }
}
