#![cfg_attr(not(debug_assertions), windows_subsystem = "windows")] // Versteckt Konsole im Release
#![allow(warnings)]
...
fn stop_old_bot() -> Result<(), Box<dyn Error>>
{
    ...
    // Terminate the old bot process (Windows)
    ...
    for entry in fs::read_dir(".")?
    {
        ...
        if let Some(file_name) = path.file_name().and_then(|n| n.to_str())
        {
            ...
            if is_generic || is_versioned
            {
                ...
                // If it's sfbot.exe, just remove it as legacy
                if is_generic
                {
