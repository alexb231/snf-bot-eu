...
function setCouponProgressText(status) {
    const textEl = document.querySelector('#coupon-progress .coupon-progress-text');
    if (!textEl) return;

    const progress = status?.progress || {};
    const processed = Number.isFinite(progress.processed) ? progress.processed : 0;
    const total = Number.isFinite(progress.total) ? progress.total : 0;
    const current = typeof progress.current === 'string' ? progress.current.trim() : '';

    if (total > 0) {
        let text = t('coupon.backgroundProgress')
            .replace('{processed}', processed.toString())
            .replace('{total}', total.toString());
        if (current) {
            text = `${text}\n${t('coupon.backgroundCurrent').replace('{name}', current)}`;
        }
        textEl.textContent = text;
        return;
    }

    textEl.textContent = t('coupon.background');
}
...
async function loadGlobalSettings() {
    try {
        ...
        document.getElementById('global-start-on-boot').checked = settings.globalStartOnBoot || false;
...
async function saveGlobalSettings() {
    try {
        const settings = {
            ...
            globalStartOnBoot: document.getElementById('global-start-on-boot').checked,
...
async function openCharacterSettings(charId, charName, options = {}) {
    ...
    if (options.useDefaults) {
        ...
        if (options.blank) {
            resetCharacterSettingsForApplyAll();
        } else {
...
function resetCharacterSettingsForApplyAll() {
    const container = document.getElementById('settings-content');
    if (!container) return;
...
async function openAllCharacterSettings() {
    ...
    await openCharacterSettings(baseCharacter.id, baseCharacter.name, { useDefaults: true, blank: true });
...
function populateCharacterSettings(settings) {
    ...
    const checkboxes = [
        ...
        'itemsCheckbox', 'itemsInventoryMinGoldSavedIgnoreGemMine', 'itemsDoNotSellEpics', 'itemsSellAfterToiletUse',
        ...
        'itemsMagicShopBuyHourglasses', 'itemsBrewPotionsUsingFruits', 'itemsEnableEquipmentSwap', 'itemsEnableMannequinSwap', 'itemsEquipBeforeSelling',
...
async function loadChangelog() {
    ...
    try {
        ...
        if (!response.ok) {
            container.innerHTML = `<div class="changelog-empty">${t('changelog.empty')}</div>`;
            return;
        }
        const html = await response.text();
        container.innerHTML = html.trim() || `<div class="changelog-empty">${t('changelog.empty')}</div>`;
    } catch (e) {
        container.innerHTML = `<div class="changelog-empty">${t('changelog.empty')}</div>`;
    }
}
